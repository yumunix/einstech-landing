        ...
        <main className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8 relative items-center py-12 lg:py-0">
          ...
          <div className="lg:col-span-7 xl:col-span-6 z-10 flex flex-col justify-center h-full pt-10 pb-20">
            <div className="flex flex-wrap items-center gap-3 mb-8">
              <div className="px-2 py-1 border border-navy/30 bg-navy/5 font-mono text-[10px] text-navy uppercase tracking-widest flex items-center gap-2">
                <span className="inline-block w-1.5 h-1.5 bg-navy" />
                Est. 1994
              </div>
            ...
            <h1 className="font-display leading-[0.85] flex flex-col gap-2 mb-8">
              ...
              <span className="text-[clamp(2.5rem,5vw,5rem)] font-light tracking-widest ds-text-glow-cyan text-navy mt-4">
                LOSSLESS DATA
              </span>
            </h1>

            <p className="font-sans text-slate-600 text-base sm:text-lg max-w-xl leading-relaxed mb-10 border-l-2 border-navy/30 pl-4 ds-word-keep">
              시스템 한 대가 다운되면 공장 라인은 분당{" "}
              <span className="text-slate-900 font-medium">280만 원</span>씩 손실을 봅니다.
          ...
          <div className="lg:col-span-5 xl:col-span-6 z-10 h-full relative hidden lg:block">
            ...
            />
            <div
              className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] border border-navy/10 rounded-full pointer-events-none border-dashed"
              aria-hidden
            />
            ...
            <GlassPanel className="absolute top-[15%] right-[5%] w-64 p-5 transform hover:-translate-y-1 transition-transform duration-500">
              <div className="ds-data-label mb-2 flex justify-between">
                ...
                <span className="text-emerald">ON</span>
              </div>
              <div className="font-display text-xs text-slate-500 mb-1">
                PREVENTED DOWNTIME LOSS (KRW)
              </div>
              <div className="font-mono text-2xl text-slate-900 font-bold flex items-center gap-2">
                <span>₩</span>
                <LossCounter />
              </div>
              <div className="mt-3 h-1 w-full bg-white rounded-full overflow-hidden">
                <div className="h-full bg-navy w-full animate-pulse-fast" />
              </div>
              <div className="mt-2 font-mono text-[9px] text-slate-500 text-right">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs text-slate-900">NODE_ALPHA [ACTIVE]</span>
                  </div>
                  <span className="font-mono text-[10px] text-navy">100%</span>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-navy rounded-full opacity-50" />
                    <span className="font-mono text-xs text-slate-500">NODE_BETA [STANDBY]</span>
                  </div>
                  <span className="font-mono text-[10px] text-slate-500">SYNCED</span>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-navy rounded-full opacity-50" />
                    <span className="font-mono text-xs text-slate-500">DATA_MIRROR</span>
                  </div>
                  <span className="font-mono text-[10px] text-slate-500">REALTIME</span>
                </div>
              </div>
              <div className="mt-4 pt-3 border-t border-white/10 flex justify-between items-center">
                <span className="font-mono text-[10px] text-slate-500">FAILOVER_READINESS</span>
                <span className="font-display text-sm text-emerald">ARMED</span>
              </div>
            </GlassPanel>

            <GlassPanel className="absolute top-[60%] right-[-5%] w-48 p-4">
              <div className="ds-data-label mb-1">SYSTEM UPTIME</div>
              <div className="font-display text-4xl text-slate-900 font-bold tracking-tighter">
                99.999<span className="text-navy text-xl">%</span>
              </div>
              <div className="mt-2 font-mono text-[9px] text-slate-500">ZERO_TOLERANCE_ZONE</div>
            </GlassPanel>
          </div>
        </main>

        <div className="h-12 border-t border-slate-200 mt-auto hidden sm:flex justify-between items-center">
          <div className="font-mono text-[10px] text-slate-500 flex gap-4">
            <span>SECURE_CONNECTION</span>
            <span>IP: 192.168.HA.01</span>
          </div>
          <div className="flex gap-1">
            <div className="w-8 h-1 bg-navy/20" />
            <div className="w-8 h-1 bg-navy/40" />
