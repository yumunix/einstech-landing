...
export function generateMetadata({ params }) {
  const service = supportServiceMap[params.slug];
  if (!service) return {};
...
export default function SupportServicePage({ params }) {
  const service = supportServiceMap[params.slug];
  if (!service) notFound();
  return <><Header /><main className="pt-16">
    <section className="relative overflow-hidden bg-slate-950 py-24 text-white lg:py-32"><div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_25%,rgba(16,185,129,.2),transparent_34%)]" /><div className="relative mx-auto w-full max-w-page px-6 lg:px-12"><Badge color="cyan">{service.code}</Badge><p className="mt-7 text-sm font-bold text-emerald-300">EINSTECH Technical Support</p><h1 className="mt-3 font-display text-[clamp(2.7rem,6vw,5.5rem)] font-black leading-none">{service.name}</h1><p className="mt-5 font-display text-xl font-bold text-white/90 sm:text-3xl">{service.subtitle}</p><p className="mt-8 max-w-3xl border-l-2 border-emerald-300/60 pl-5 text-base leading-8 text-white/70">{service.summary}</p></div></section>
