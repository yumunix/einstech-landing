...
export default function HpServerPage() {
  return <><Header /><main className="pt-16">
    <section className="py-24 lg:py-32"><div className="mx-auto w-full max-w-page px-6 lg:px-12"><Badge color="cyan">HP_SERVER</Badge><h1 className="mt-7 font-display text-[clamp(2.7rem,5vw,5rem)] font-black leading-none text-slate-900">HP Server</h1><p className="mt-4 font-display text-xl font-bold text-navy sm:text-3xl">Enterprise Compute</p><p className="mt-8 max-w-3xl border-l-2 border-navy/30 pl-5 text-base leading-8 text-slate-600">최신 HPE ProLiant Compute Gen12와 Synergy, 고밀도 Apollo 플랫폼을 워크로드에 맞춰 설계합니다. CPU·메모리·스토리지·네트워크·이중화 구성을 검토해 검증된 BOM과 공급 견적을 제공합니다.</p><div className="mt-10 flex flex-col gap-3 sm:flex-row"><PrimaryButton href="#lineup">대표 제품 보기</PrimaryButton><SecondaryButton href="/contact?subject=HPE%20Server%20구성%20상담">구성 상담</SecondaryButton></div></div></section>
