...
export default function RackAdvancedConfigurator() {
  ...
  const inquiry = encodeURIComponent("EINSWALL RACK ADVANCED / Intel Core i5-13400 / 8GB / 정품 500GB NVMe 구성 견적 문의");
    ...
    <div className="grid overflow-hidden rounded-[32px] border border-slate-200 bg-white lg:grid-cols-[1fr_360px] lg:ml-12">
      <div className="p-7 sm:p-9">
        <div className="mb-6 flex flex-wrap items-center gap-3">
          <span className="rounded-full bg-emerald/10 px-3 py-1 font-mono text-[10px] font-bold tracking-widest text-emerald">RACK ADVANCED</span>
          <span className="font-mono text-[10px] text-slate-400">19형 1U · 고급형</span>
        </div>
        ...
        <dl className="mt-7 grid gap-3 sm:grid-cols-2">
          {[["CPU", "Intel Core i5-13400 · 10코어"], ["폼팩터", "19형 1U Rackmount"], ["네트워크", "8×2.5GbE"], ["광 포트", "4×10G SFP+"], ["메모리", "8GB 구성 기준 · 최대 32GB"], ["스토리지", "정품 500GB 권장 · 250GB 재고 확인"]].map(([label, value]) => (
        ...
        <p className="mt-5 text-[11px] leading-relaxed text-slate-400">CPU·포트 표기는 공급 원제품의 공개 사양 기준입니다. 실제 납품 사양과 부품 재고는 발주 시점에 다시 확인합니다. Core i7-13700·Core i9-13900 구성은 별도 견적입니다.</p>
      </div>
      <aside className="bg-navy p-8 text-white">
        ...
        <div className="mt-5 space-y-3 border-y border-white/10 py-5 text-sm text-white/75">
          {["Intel Core i5-13400", "삼성 8GB 메모리", "삼성 980 / SK하이닉스 Gold P31 500GB 정품", "기본 방화벽 OS 설치"].map((item) => <div key={item} className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-emerald" />{item}</div>)}
        </div>
        ...
        <div className="mt-2 font-mono text-[10px] leading-relaxed text-white/40">원제품 US$763.88 · 고정환율 {won.format(pricingPolicy.fixedUsdKrw)}원 · 수입비용 {(pricingPolicy.importTaxRate * 100).toFixed(0)}% · 만원 단위 올림<br />가격 기준 {pricingPolicy.effectiveMonth}</div>
        ...
        <a href={`/contact?subject=${inquiry}`} className="mt-6 block rounded-2xl bg-emerald px-5 py-4 text-center font-bold text-white transition-colors hover:bg-white hover:text-navy">랙형 고급형 견적 문의</a>
      </aside>
    </div>
  );
}
