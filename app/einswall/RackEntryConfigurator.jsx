...
export default function RackEntryConfigurator() {
  ...
  const inquiry = encodeURIComponent("EINSWALL RACK ENTRY / Intel Atom C3758 / 16GB / 500·512GB NVMe 구성 견적 문의");
    ...
    <div className="grid overflow-hidden rounded-[32px] border border-slate-200 bg-white lg:grid-cols-[1fr_360px] lg:ml-12">
      <div className="p-7 sm:p-9">
        ...
        <dl className="mt-7 grid gap-3 sm:grid-cols-2">
          {[["CPU", "Intel Atom C3758 · 8코어"], ["폼팩터", "19형 1U Rackmount"], ["네트워크", "6×1GbE + 4×2.5GbE"], ["광 포트", "4×SFP"], ["메모리", "16GB 구성 기준"], ["스토리지", "500/512GB NVMe 구성 기준"]].map(([label, value]) => (
      ...
      <aside className="bg-navy p-8 text-white">
        ...
        <div className="mt-5 space-y-3 border-y border-white/10 py-5 text-sm text-white/75">
          {["Atom C3758", "16GB 메모리", "500/512GB NVMe", "기본 방화벽 OS 설치"].map((item) => <div key={item} className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-emerald" />{item}</div>)}
        </div>
