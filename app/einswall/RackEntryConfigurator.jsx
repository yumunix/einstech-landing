...
export default function RackEntryConfigurator() {
  ...
  const inquiry = encodeURIComponent("EINSWALL RACK ENTRY / Intel Atom C3758 / 8GB / 128GB NVMe 구성 견적 문의");
    ...
    <div className="grid overflow-hidden rounded-[32px] border border-slate-200 bg-white lg:grid-cols-[1fr_360px] lg:ml-12">
      <div className="p-7 sm:p-9">
        ...
        <dl className="mt-7 grid gap-3 sm:grid-cols-2">
          {[["CPU", "Intel Atom C3758 · 8코어"], ["폼팩터", "19형 1U Rackmount"], ["네트워크", "6×1GbE + 4×2.5GbE"], ["광 포트", "4×SFP"], ["메모리", "8GB 구성 기준 · 최대 32GB"], ["스토리지", "128GB NVMe 기준 · 최대 512GB"]].map(([label, value]) => (
        ...
        <p className="mt-5 text-[11px] leading-relaxed text-slate-400">CPU·포트 표기는 공급 원제품의 공개 사양 기준입니다. 실제 납품 사양과 부품 재고는 발주 시점에 다시 확인합니다. 랙형 고급 모델은 별도 등록 예정입니다.</p>
      </div>
      <aside className="bg-navy p-8 text-white">
        ...
        <div className="mt-5 space-y-3 border-y border-white/10 py-5 text-sm text-white/75">
          {["Atom C3758", "삼성 8GB 메모리", "삼성 128GB NVMe", "기본 방화벽 OS 설치"].map((item) => <div key={item} className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-emerald" />{item}</div>)}
        </div>
