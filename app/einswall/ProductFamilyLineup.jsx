...
import { calculateInstallment } from "./subscription";
        ...
        <section key={group.id} className="border-t border-slate-200 pt-7 first:border-t-0 first:pt-0">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {group.products.map((product) => {
              const installment = calculateInstallment(product.price, 40, 36);
              return (
              <article key={product.name} className="group overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                ...
                <div className="p-5"><div className="font-mono text-[9px] font-bold tracking-widest text-emerald">{product.chip}</div><h4 className="mt-2 font-display text-lg font-black text-slate-900">{product.name}</h4><p className="mt-1 min-h-9 text-xs leading-relaxed text-slate-500">{product.spec}</p><div className="mt-4 grid grid-cols-2 gap-2 border-t border-slate-100 pt-4"><div className="rounded-xl bg-slate-50 p-3"><div className="text-[9px] text-slate-500">일시불 · VAT 별도</div><strong className="mt-1 block font-display text-base text-navy">{won.format(product.price)}원~</strong></div><div className="rounded-xl bg-cyan-50 p-3"><div className="text-[9px] text-cyan-800">40% 선납·3년 구독 · VAT 별도</div><strong className="mt-1 block font-display text-base text-cyan-900">월 {won.format(installment.monthly)}원~</strong><div className="mt-1 text-[9px] font-bold text-cyan-800">선납 {won.format(installment.downPayment)}원</div></div></div><div className="mt-4 flex justify-end"><a href={`#pricing-${group.id}`} onClick={() => window.dispatchEvent(new CustomEvent("einswall:model-select", { detail: { family: group.id, model: product.name.split(" ").at(-1).toLowerCase() } }))} className="rounded-full bg-slate-100 px-3 py-2 text-[10px] font-bold text-navy group-hover:bg-navy group-hover:text-white">결제 방식·사양 선택</a></div></div>
              </article>
              );
            })}
          </div>
