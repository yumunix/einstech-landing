          ...
          <div className="grid gap-5 md:grid-cols-3">
            {group.products.map((product) => {
              ...
              <article key={product.name} className="group overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                ...
                <div className="p-5"><div className="font-mono text-[9px] font-bold tracking-widest text-emerald">{product.chip}</div><h4 className="mt-2 font-display text-lg font-black text-slate-900">{product.name}</h4><p className="mt-1 min-h-9 text-xs leading-relaxed text-slate-500">{product.spec}</p><div className="mt-4 grid grid-cols-2 gap-2 border-t border-slate-100 pt-4"><div className="rounded-xl bg-slate-50 p-3"><div className="text-[9px] text-slate-500">일시불 · VAT 별도</div><strong className="mt-1 block font-display text-base text-navy">{won.format(product.price)}원~</strong></div><div className="rounded-xl bg-emerald/10 p-3"><div className="text-[9px] text-emerald-700">3년 구독 · 월/VAT별도</div><strong className="mt-1 block font-display text-base text-emerald-700">{won.format(subscription.monthly)}원~</strong></div><div className="col-span-2 rounded-xl bg-cyan-50 p-3"><div className="text-[9px] text-cyan-800">40% 선납 + 36개월 분할 · VAT 별도</div><div className="mt-1 flex flex-wrap items-end justify-between gap-2"><strong className="font-display text-base text-cyan-900">선납 {won.format(installment.downPayment)}원</strong><strong className="font-display text-base text-cyan-900">월 {won.format(installment.monthly)}원~</strong></div></div></div><div className="mt-4 flex justify-end"><a href={`#pricing-${group.id}`} onClick={() => window.dispatchEvent(new CustomEvent("einswall:model-select", { detail: { family: group.id, model: product.name.split(" ").at(-1).toLowerCase() } }))} className="rounded-full bg-slate-100 px-3 py-2 text-[10px] font-bold text-navy group-hover:bg-navy group-hover:text-white">결제 방식·사양 선택</a></div></div>
              </article>
              );
            })}
          </div>
